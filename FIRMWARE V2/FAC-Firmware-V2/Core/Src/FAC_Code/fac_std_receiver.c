...
static void FAC_std_receiver_is_connected(void);
/* FUNCTION DEFINITION */
/**
 * @brief 	check all rx channels, and if at least one is != 0 it means that the receiver has been connected
 * @note	sets the std receiver flag
 */
static void FAC_std_receiver_is_connected(void) {
	/* IMPORTANT NOTE! */
	/* NOTE this can have some problem with some of the receiver outhere */
	...
	if (!receiver.connected) {	// I only check if it is not connected yet
		...
		receiver.connected = receiverConnected;
	}
}
 ...
 *
 */
static int16_t FAC_std_receiver_calculate_dead_zone(uint16_t value,
		uint8_t deadzonePerc, uint8_t chNumber) {
	...
	if (chNumber != 3) { // channel 3 always don't have the return spring, so the offset must be only at the extremes
		temp = map_int32(temp, 0, RECEIVER_CHANNEL_RESOLUTION,
				-RECEIVER_CHANNEL_RESOLUTION, RECEIVER_CHANNEL_RESOLUTION); // change the range of the value
	}
	...
	/* linearization of the value if it is not in the center */
		...
		temp = map_int32(temp, 0 + deadzoneValue,
				maxValue - (deadzoneValue * 2), 0, maxValue);
	else if (temp < 0)	// linearize the bottom value
		temp = map_int32(temp, minValue + (deadzoneValue * 2),
				0 - deadzoneValue, minValue, 0);

	if (chNumber != 3) {
		temp = map_int32(temp, -RECEIVER_CHANNEL_RESOLUTION,
		RECEIVER_CHANNEL_RESOLUTION, 0, RECEIVER_CHANNEL_RESOLUTION);
	}
